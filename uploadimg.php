...
if (!is_uploaded_file(@$this->fileName[tmp_name]))
   //是否存在文件
   {
   echo "<script>alert('请点击“浏览”，先选择您要上传的文件！\\n\\n支持的图片类型为：jpg,gif,png,bmp');parent.window.close();</script>";
   exit;
}
if ($this->max_file_size*1024 < $this->fileName["size"])
   //检查文件大小
   {
   echo "<script>alert('文件大小超过了限制！最大只能上传 ".$this->max_file_size." K的文件');parent.window.close();</script>";
   exit;
}
//检查文件类型
if (!in_array($this->fileName["type"], $this->uptypes)) {//这种通过在文件头加GIF89A，可骗过
   echo "<script>alert('文件类型错误，支持的图片类型为：jpg,gif,png,bmp');parent.window.close();</script>";
   exit;
}
...
unlink($newName);
echo "<script>alert('经判断上传的文件不是图片文件，已删除。');parent.window.close();</script>";
exit;
} 
   ...
   if($this->imgpreview == 1 && $data[2]!=4){//文件类型不为4，4为swf格式
    switch ($data[2]) {
     ...
     case 6 :
      ...
      break;
     default :
     echo "<script>alert('不支持的文件类型，无法生成缩略图');parent.window.close();</script>";
      exit;
    }
//生成小图
if ($data[1]>$data[0])
   ...
   //如上传广告页不能加水印，很小的不加水印,3.png无法加，4.swf不能加,6.bmp无法加
    ...
    switch ($imginfo[2]) {
     ...
     case 6 :
      ...
      break;
     default :
     echo "<script>alert('不支持的文件类型，无法加水印');window.close();</script>";
      exit;
    }
    ...
    switch ($imginfo[2]) {
     case 1 :
      imagegif($nimage, $newName,98);
      break;
     case 2 :
      imagejpeg($nimage, $newName,98);
      break;
     case 3 :
      imagepng($nimage, $newName,98);
      break;
     case 6 :
      imagewbmp($nimage, $newName,98);     //98 为打印水印后图片的质量
      break;
    }
    //覆盖原上传文件
    imagedestroy($nimage);
