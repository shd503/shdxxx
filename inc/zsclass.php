	...
	if (cache_update_time!=0 && file_exists($fp) && time()-filemtime($fp)<3600*24*cache_update_time ) {//12小时更新一次,
		...
		if ($row){
			...
			while ($row=mysql_fetch_array($rs)){
				...
				$str=$str. "<h2>";
				if ($row["img"]<>'0'){$str=$str. "<img src=".str_replace('{#siteskin}',$siteskin,$row["img"]).">&nbsp;";}
				...
				if ($rown){
					while ($rown=mysql_fetch_array($rsn)){
						...
						if ($num_p<>0){
							...
							$nnn=1;
							$sqlcp="select id,proname,editor from zzcms_main where bigclasszm='".$row["classzm"]."' and smallclasszm='".$rown["classzm"]."' order by sendtime desc limit 0,$num_p";
							$rscp=mysql_query($sqlcp);
							$rowcp=mysql_num_rows($rscp);
							if ($rowcp){
								while ($rowcp=mysql_fetch_array($rscp)){
									$str=$str. "<a href='".getpageurlzs($rowcp['editor'],$rowcp['id'])."' target='_blank'>".cutstr($rowcp['proname'],$long_p)."</a>";
