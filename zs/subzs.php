...
$cpid=$_COOKIE["zzcmscpid"];
	if (strpos($cpid,",")>0){
		...
		$sql="select id,proname,img,editor from zzcms_main where id in (".$cpid.")";
	}else{
		$sql="select id,proname,img,editor from zzcms_main where id='$cpid' ";
	}	
$n=1;
$str="<table width=100% border=0 cellspacing=0 cellpadding=4><tr>";	
$rs=mysql_query($sql);
...
$str=$str. "<a href='".getpageurl("zs",$row["id"])."' target='_blank'>";//链接与上面不同
$str=$str. "<img src=".str_replace(".","_small.",$row["img"])." border='0' onload='resizeimg(".$imgwidth.",".$imgheight.",this)' alt='".$row["proname"]."'>";
$str=$str. "</a></td></tr>";
$str=$str. "<tr>";
$str=$str. "<td>".cutstr($row["proname"],$title_num)."</td>";
$str=$str. "<tr/>";
$str=$str. "</table>";
$str=$str. "</td>";
if ($n % $column==0 ){
...
$str=$str. "</table>";
$str=$str. "<div style='text-align:center;font-weight:bold'><a href='/zs/zs.php?action=clear_cookies'>清空查看记录</a></div>";
}
return $str;
}
...
function zsCompanyInB($classid){
$sql = "select  userid,comane from zzcms_main where bigclasszm='" . $classid . "' and passed=1 group by userid,comane order by userid desc limit 0,10";
$rs=mysql_query($sql);
$row=mysql_num_rows($rs);
if (!$row){
$str="暂无信息";
...
$str = $str . "<li>";
if (whtml == "Yes") {
	$str = $str . "<a href='/zt/show-" . $row["userid"] . ".htm' target='_blank'>";
	}else{
	$str = $str . "<a href='/zt/show.php?id=" . $row["userid"] . "' target='_blank'>";
	}
$str = $str . "</li>";
}
$str = $str . "</ul>";
...
$n=1;
$str="";
	$sql="select id,proname,sendtime,passed,elite,hit,comane,userid,editor from zzcms_main where passed=1 ";
	if ($classname<>"") {
...
$str=$str."<a href='".getpageurlzs($row["editor"],$row["id"])."' target='_blank' title='".$row["proname"]."'>".cutstr($row["proname"],$strnum)."</a>";
$str=$str."</li>";
$str=$str."<li id=zsLayer$n style='display:none;background:url(/image/k.gif) no-repeat 0px 0px;height:50px;padding:5px' onMouseOver=\"showfilter2(zsLayer$n)\" onMouseOut=\"showfilter2(zsLayer$n)\">".cutstr($row["comane"],12)."<br><a href='".getpageurlzs($row["editor"])."' target='_blank'><b>了解详请</b></a></li>";		
$n++;
}        
...
function showzsorder($num,$strnum,$keyword){
$n=1;
$str="";
$sql="select saver,cp from zzcms_dl where passed=1 ";
if ($keyword<>false) {
$sql=$sql. " and cp like '%".$keyword."%' ";
}

$sql=$sql. " group by saver order by count(*) desc ";
$sql=$sql." limit 0,$num";
$rs=mysql_query($sql);
$row=mysql_num_rows($rs);
if (!$row){
...
//}
$str=$str. "<div>";
	$sqln="select proname,editor,img,prouse from zzcms_main where editor='".$row["saver"]."' ";
	...
	$str=$str. "<a href='".getpageurlzs($r['editor'])."' target='_blank'>".cutstr($r['proname'],$strnum)."</a>";
	$str=$str. "</li>";
...
$str=$str."<img src='".getsmallimg($row["img"])."'>";
$str=$str."</a>";
$str=$str."</td></tr></table>";
}
