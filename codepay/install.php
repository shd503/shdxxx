...
(1, 'admin', '0.00', 0, 0);");
    ...
    /*    if (function_exists("preg_replace_callback")) { //PHP5.5以上
            $configString = preg_replace_callback(
                '/\"([^\"]*)\"/', function ($matches){return get_value($matches[1]);}
                , $modeStr); //配置模板替换
        }else{*/
    $configString = preg_replace( //PHP5.4以下
        '/\"([^\"]*)\"/es',
        "get_value('\\1')",
        $modeStr
    ); //配置模板替换
    ...
    <div
        ...
        <div ng-show="page === 'installed'" id="installed" class="panel panel-default install-panel">
            <div class="panel-body">
                ...
                <button type="button" class="btn btn-success"
                        onclick="window.open('https://codepay.fateqq.com/admin/#/dataSet.html?herf='+encodeURIComponent(window.location))">
                    设置通知地址
                </button>
                <br>
            </div>
