...
$sql="select id,title,content,elite,sendtime,img,link,hit from zzcms_zx where passed=1"; 
$sql=$sql.$sql2;
$sql=$sql." order by elite desc,id desc limit $offset,$page_size";
//$sql=$sql." and id>=(select id from zzcms_zx where  bigclassid='".$b."' order by id desc limit $offset,1) limit $page_size";
$rs = mysql_query($sql); 
if(!$totlenum){
