...
$ml='../uploadfiles/'.$mlname;
	...
	$file="/uploadfiles/".$mlname.'/'.$file;
		...
		$rs=mysql_query("select bodybg from zzcms_main where bodybg='".$file."' or bodybg='".str_replace("_small.",".",$file)."'");
    	$row=mysql_num_rows($rs);
	    if (!$row){
		$a3=0;//zzcms_main表中没有用到这个图片
		}else{
		$a3=1;
		}
		...
		if ($a+$a1+$a2+$a3+$b+$c+$c1+$c2+$d+$e+$e1+$f+$f1+$i+$j==0) {//如果在这几个表中都没有用到这个图片
