...
$f="../template/".siteskin."/label/helpshow/".$title.".txt";
$fp=fopen($f,"w+");//fopen()的其它开关请参看相关函数
$str=$title . "|||".$elite . "|||" . $numbers . "|||" . $orderby ."|||" . $titlenum ."|||" . $cnum ."|||" . $row . "|||" . $start . "|||" . $mids . "|||" . $ends;
fputs($fp,$str);
fclose($fp);
...
if (file_exists("../template/".siteskin."/label/helpshow")==false){
echo '文件不存在';
}else{		
$dir = opendir("../template/".siteskin."/label/helpshow");
...
//读取现有标签中的内容
if ($labelname!=''){
$fp="../template/".siteskin."/label/helpshow/".$labelname;
$f=fopen($fp,"r+");
$fcontent="";
while (!feof($f))
{
...
<input name="title_old" type="hidden" id="title_old" value="<?php echo $title?>" size="50" maxlength="255">      </td>
    ...
    <tr> 
      ...
      <td class="border" ><input name="elite[]" type="checkbox" id="elite" value="1" <?php if ($elite==1) { echo " checked";}?>>
        推荐的 </td>
    </tr>
    <tr> 
